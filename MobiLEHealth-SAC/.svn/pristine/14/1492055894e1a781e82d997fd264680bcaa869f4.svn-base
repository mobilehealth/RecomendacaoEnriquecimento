...
/**
 * Implements Map interface in using less memory. Very simple but usefull for small number of items
 * on it.
 */

public class MetadataMap implements Map<Object, Object>, java.lang.Cloneable, java.io.Serializable {
	...
	/**
	 * Array keeping the keys of the entries in the map. It is "synchrnized" with the values array -
	 * the Nth position in both arrays correspond to one and the same entry
	 */
	Object theKeys[];

	/**
	 * Array keeping the values of the entries in the map. It is "synchrnized" with the keys array -
	 * the Nth position in both arrays correspond to one and the same entry
	 */
	Object theValues[];
	...
	/**
	 * put a value in the map using the given key. If the key exist in the map the value is replaced
	...
	/**
	 * return positive value as index of the key in the map. Negative value means that the key is
	...
	/**
	 * return the index of the key in the map comparing them by reference only. This method is used
	 * in subsume check to speed it up.
	 */
