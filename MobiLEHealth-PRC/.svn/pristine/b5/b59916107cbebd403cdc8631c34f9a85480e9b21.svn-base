...
public class RecomendacaoConjunta {
    ...
    private void iniciarPopulacao(ArrayList<RelatePersonContent> resultadoRecomendacaoHibrida) {
        ...
        melhorRepresentacao = new ArrayList<RelatePersonContent>();
        ...
        for(int i = 0; i < tamPopulacao; i++){
            ...
            for(int j = 0; j < tamCromossomo; j++){
            	...
            	//System.out.println("Algoritmo Genetico tamanho da lista = " + resultadoRecomendacaoHibrida.size());
	...
	private void mutar(){
		...
		// ***********************************************************
			//Qual o valor da taxa de mutação?
        //***********************************************************
		int taxaMutacao;
        ...
        for(int cont = 1; cont < populacao.size(); cont++){
            for(int cont2 = 0; cont2 < populacao.get(cont).size(); cont2++){
                ...
                if(random.nextInt(1000) < taxaMutacao){
					...
					/****************************************************************
                	 * 
                	 * Qual o valor que eu comparo com 0?
                	 * 
                	 * Código Original: if(populacao[cont][cont2] == 0){
                	 * 
                	 * Modifiquei para comparar com o status
                	 * 
                	 */
    
    /*
                     if(populacao.get(cont).get(cont2).getStatus() == 0){
                    	
                    	
                    	/****************************************************************
                    	 * 
                    	 * Qual a variavel que eu seto o valor ?
                    	 * 
                    	 * Código Original: populacao[cont][cont2] = 1;
                    	 * 
                    	 * Modifiquei para o metodo de sugestões                    	 * 
                    	 */
    
    /*
                        populacao.get(cont).get(cont2).setSuggestedMethod(1);
                        ...
                        if(auxTempo[cont] > this.tempo || auxCusto[cont] > this.dinheiro){//verifica se a solução representada pelo cromossomo é consistente a cada novo gene definido.
                        	 ...
                        	 * Qual a variavel que eu seto o valor ?
                        	 * 
                        	 * Código Original: populacao[cont][cont2] = 0;
                        	 * 
                        	 * Modifiquei para o metodo de sugestões                    	 * 
                        	 */
      
    /*
    						populacao.get(cont).get(cont2).setSuggestedMethod(0);//caso não seja, elimina o último elemento inserido e encerra o laço.
    ...
    */
    ...
    public ArrayList<RelatePersonContent> algoritmoGenetico(ArrayList<RelatePersonContent> resultadoRecomendacaoHibrida){
    	...
    	if(resultadoRecomendacaoHibrida.size() == 0){
