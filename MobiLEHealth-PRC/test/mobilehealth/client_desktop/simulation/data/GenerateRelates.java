...
public class GenerateRelates {
	...
	private void geraRelacoesPersonPersonLocation(double taxaPersonPerson) {
		...
		int tamanhoFaixaPerson = (int) (sizePerson/numeroDeFaixas);
		...
		for(int i = 0; i < sizePerson; i++){
			...
			for(int j = 0; j < sizePerson*taxaPersonPerson; j++){
				int k = 0;
				if (random.nextFloat() < 0.8){
					k = random.nextInt(tamanhoFaixaPerson) + tamanhoFaixaPerson*numeroDaFaixa;
					location = locais.get(random.nextInt(tamanhoFaixaLocation) + (int) (tamanhoFaixaLocation*numeroDaFaixa));					
				} else {
					k = random.nextInt(sizePerson);
					location = locais.get(random.nextInt(sizeLocation));										
				}					
