...
//			adequacaoFrequency[j] -= listRelateContent.get(j).getContent().getFrequency().getH7NegativeV()[history.getTimeStamp().DAY_OF_WEEK];
			...
			if(history.getPersonSpeed() > 20 ) {
				if (listRelateContent.get(j).getContent().getSubtype() == 0)	adequacaoSituacao[j] += -10;
				else if (listRelateContent.get(j).getContent().getSubtype() == 1)	adequacaoSituacao[j] += -5;	
				else if (listRelateContent.get(j).getContent().getSubtype() == 2)	adequacaoSituacao[j] += 5;
				else if (listRelateContent.get(j).getContent().getSubtype() == 3)	adequacaoSituacao[j] += 10;
			}
			...
			for(int k = 0; k < listRelateContent.get(j).getContent().getListRelateContentLocation().size(); k++) {
				if(listRelateContent.get(j).getContent().getListRelateContentLocation().get(k).getLocation().getId() == history.getLocation().getId()) {
					if (listRelateContent.get(j).getContent().getListRelateContentLocation().get(k).getStatus() == RelateContentLocation.STATUS_ABOUT) adequacaoLocations[j] += 200;
					else if (listRelateContent.get(j).getContent().getListRelateContentLocation().get(k).getStatus() == RelateContentLocation.STATUS_CITED) adequacaoLocations[j] += 20;
					else if (listRelateContent.get(j).getContent().getListRelateContentLocation().get(k).getStatus() == RelateContentLocation.STATUS_RELATED) adequacaoLocations[j] += 5;
				}
			}
